...
import { Check, Trash2, Clock, Play, Pause, Square } from 'lucide-react';
...
const TaskRow: React.FC<{
  ...
  onStart: (id: string) => void; onPause: (id: string) => void; onStop: (id: string) => void;
}> = ({ task, index, onToggle, onDelete, onStart, onPause, onStop }) => {
  ...
  return (
    <div className={`glass-card-hover rounded-xl p-4 animate-slide-up ${task.completed ? 'opacity-60' : ''}`} style={{ animationDelay: `${index * 40}ms` }}>
      <div className="flex items-center gap-3">
        <button onClick={() => onToggle(task.id)}
          className={`w-6 h-6 rounded-md border-2 flex items-center justify-center flex-shrink-0 transition-all ${task.completed ? 'bg-primary border-primary' : 'border-muted-foreground/30 hover:border-primary'}`}>
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2">
            ...
            <span className={`font-medium text-sm ${task.completed ? 'line-through text-muted-foreground' : 'text-foreground'}`}>{task.name}</span>
          </div>
          <div className="flex items-center gap-2 mt-1 flex-wrap">
            ...
            <span className="text-xs font-medium px-1.5 py-0.5 rounded" style={{ backgroundColor: pri.color + '20', color: pri.color }}>{pri.label}</span>
            <span className="text-xs font-medium px-1.5 py-0.5 rounded" style={{ backgroundColor: urg.color + '20', color: urg.color }}>{urg.label}</span>
          </div>
        </div>
        ...
        {!task.completed && (
          <div className="flex items-center gap-1.5 flex-shrink-0">
            <span className={`text-xs font-mono font-semibold min-w-[52px] text-right ${task.timerStatus === 'running' ? 'text-primary' : 'text-muted-foreground'}`}>
              {formatSeconds(elapsed)}
            </span>
            {task.timerStatus === 'idle' && (
              <button onClick={() => onStart(task.id)} className="p-1.5 rounded-md hover:bg-primary/10 text-primary transition-colors" title="Start timer">
                <Play size={14} />
              </button>
            )}
            {task.timerStatus === 'running' && (
              <>
                <button onClick={() => onPause(task.id)} className="p-1.5 rounded-md hover:bg-warning/10 text-warning transition-colors" title="Pause">
                  <Pause size={14} />
                </button>
                <button onClick={() => onStop(task.id)} className="p-1.5 rounded-md hover:bg-danger/10 text-danger transition-colors" title="Stop">
                  <Square size={14} />
                </button>
              </>
            )}
            {task.timerStatus === 'paused' && (
              <>
                <button onClick={() => onStart(task.id)} className="p-1.5 rounded-md hover:bg-primary/10 text-primary transition-colors" title="Resume">
                  <Play size={14} />
                </button>
                <button onClick={() => onStop(task.id)} className="p-1.5 rounded-md hover:bg-danger/10 text-danger transition-colors" title="Stop">
                  <Square size={14} />
