    ...
    <div className="glass-card rounded-xl p-5 animate-fade-in">
      ...
      <div className="relative" style={{ height: '600px' }}>
        ...
        <div className="absolute left-12 right-0 top-0 bottom-0">
          {dayTasks.map(task => {
            const pos = getPos(task.plannedStart, task.plannedEnd);
            const color = PRIORITY_CONFIG[task.priority].color;
            return (
              <div key={task.id} className={`absolute left-1 right-1 rounded-lg px-3 py-1.5 text-xs font-medium overflow-hidden ${task.completed ? 'opacity-50' : ''}`}
                style={{ ...pos, backgroundColor: color + '20', borderLeft: `3px solid ${color}`, color: 'hsl(var(--foreground))' }}>
                <div className="truncate font-semibold">{task.name}</div>
                <div className="text-muted-foreground">{task.plannedStart} – {task.plannedEnd}</div>
              </div>
            );
          })}
        </div>
